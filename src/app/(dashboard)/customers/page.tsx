...
interface Customer {
    ...
    creditLimit: number
    creditBalance: number   // saldo deudor (positivo = debe)
    totalSpent: number
    visitCount: number
    lastVisit: string
    since: string
...
const AVATAR_COLORS = [
    '#f5a623', '#34d399', '#60a5fa', '#a78bfa', '#f87171', '#fb923c', '#38bdf8',
]
...
export default function CustomersPage() {
    ...
    const filtered = customers.filter(c => {
        ...
        const matchQ = c.name.toLowerCase().includes(q) || c.phone.includes(q)
    ...
    const handleCharge = (customerId: string, amount: number, note: string) => {
        setCustomers(prev => prev.map(c => c.id !== customerId ? c : {
            ...c,
            creditBalance: c.creditBalance + amount,
            creditHistory: [...c.creditHistory, {
                id: String(Date.now()), date: new Date().toLocaleDateString('es-CR'),
        ...
        setSelected(prev => prev ? {
            ...
            creditHistory: [...prev.creditHistory, {
                id: String(Date.now()), date: new Date().toLocaleDateString('es-CR'),
                type: 'charge', amount, note,
            }],
        } : null)
    ...
    return (
        <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
            ...
            <div style={{
                ...
                {/* Stats */}
                <div style={{ padding: '20px 20px 0', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px' }}>
                    {[
                        { label: 'Clientes', value: customers.length, color: 'var(--info)', icon: <Users size={14} /> },
                        { label: 'Fiado total', value: formatCRC(totalCredit), color: 'var(--warning)', icon: <CreditCard size={14} />, mono: true },
                ...
                <div style={{ padding: '14px 20px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <div style={{ position: 'relative', flex: 1 }}>
                        ...
                        <input className="input-base" value={search} onChange={e => setSearch(e.target.value)}
                            placeholder="Nombre o teléfono..." style={{ width: '100%', padding: '8px 10px 8px 28px', fontSize: '13px' }} />
                    </div>

                    <select className="input-base" value={filterStatus} onChange={e => setFilterStatus(e.target.value as typeof filterStatus)}
                    ...
                    <button onClick={() => { setEditing(null); setShowModal(true) }} className="btn-accent"
                        style={{ padding: '8px 14px', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '5px', flexShrink: 0 }}>
                        <Plus size={14} /> Nuevo
                    </button>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {filtered.map(c => (
                        <CustomerRow
                            ...
                            onClick={() => { setSelected(c); setActiveTab('history') }}
                        />
                    ))}
                    {filtered.length === 0 && (
...
// ─── CustomerRow ──────────────────────────────────────────────────────────────
function CustomerRow({ customer: c, isSelected, onClick }: {
    customer: Customer; isSelected: boolean; onClick: () => void
}) {
    ...
    return (
        ...
        >
            ...
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
                {c.creditBalance > 0 && (
                    <div style={{ fontSize: '12px', fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--warning)' }}>
                        {formatCRC(c.creditBalance)}
                    </div>
                )}
...
// ─── CustomerDetail ───────────────────────────────────────────────────────────
function CustomerDetail({ customer: c, activeTab, onTabChange, onClose, onEdit, onCharge, onPayment }: {
    customer: Customer
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
                    <div style={{
                        ...
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: '16px', fontWeight: 700, color, flexShrink: 0,
                    }}>
                        {initials(c.name)}
                    </div>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                            ...
                            <span><Calendar size={11} style={{ display: 'inline', marginRight: 3 }} />Cliente desde {c.since}</span>
                        </div>
                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '8px', marginTop: '16px' }}>
                    {[
                        { label: 'Total gastado', value: formatCRC(c.totalSpent), color: 'var(--accent)', mono: true },
                        ...
                        <div key={s.label} style={{
                            background: 'var(--bg-elevated)', borderRadius: 'var(--radius-md)', padding: '10px 12px',
                        }}>
                            <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '4px' }}>{s.label}</div>
                            <div style={{ fontSize: '13px', fontWeight: 700, color: s.color, fontFamily: s.mono ? 'var(--font-mono)' : 'inherit' }}>
                                {s.value}
                            </div>
                        </div>
                    ))}
                    ...
                    <div style={{ marginTop: '14px', background: 'var(--bg-elevated)', borderRadius: 'var(--radius-md)', padding: '12px 14px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '8px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '12px', alignItems: 'baseline' }}>
                                <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                                    Límite: {formatCRC(c.creditLimit)}
                                </span>
                                <span style={{
                                    fontSize: '15px', fontWeight: 700, fontFamily: 'var(--font-mono)',
                                    color: c.creditBalance > 0 ? 'var(--warning)' : 'var(--success)',
                                }}>
                                    {c.creditBalance > 0 ? `Debe: ${formatCRC(c.creditBalance)}` : '✓ Al día'}
                                </span>
                            </div>
                        </div>
                        {/* Barra de progreso */}
                        <div style={{ height: 5, background: 'var(--bg-overlay)', borderRadius: 3, overflow: 'hidden' }}>
                            <div style={{
                                ...
                                width: `${creditPct}%`,
                                background: creditPct > 80 ? 'var(--danger)' : creditPct > 50 ? 'var(--warning)' : 'var(--success)',
                            }} />
                        </div>
                        <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                            <button onClick={() => { setCreditType('payment'); setShowCreditForm(true) }}
                                style={{
                                    flex: 1, padding: '7px', borderRadius: 'var(--radius-md)', fontSize: '12px', fontWeight: 500,
                                    background: 'var(--success-bg)', border: '1px solid rgba(52,211,153,0.3)',
                                    color: 'var(--success)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px',
                                }}>
                                <ArrowDownRight size={13} /> Registrar pago
                            </button>
                            <button onClick={() => { setCreditType('charge'); setShowCreditForm(true) }}
                                style={{
                                    flex: 1, padding: '7px', borderRadius: 'var(--radius-md)', fontSize: '12px', fontWeight: 500,
                                    background: 'var(--danger-bg)', border: '1px solid rgba(248,113,113,0.3)',
                                    color: 'var(--danger)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px',
                                }}>
                        ...
                        {showCreditForm && (
                            <div style={{ marginTop: '10px', padding: '12px', background: 'var(--bg-overlay)', borderRadius: 'var(--radius-md)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                <input type="number" className="input-base" value={amount} onChange={e => setAmount(e.target.value)}
                                    placeholder="Monto en ₡" style={{ padding: '8px 12px', fontSize: '16px', fontFamily: 'var(--font-mono)', fontWeight: 600 }} />
                                <input className="input-base" value={note} onChange={e => setNote(e.target.value)}
                                    placeholder="Descripción (opcional)" style={{ padding: '8px 12px', fontSize: '13px' }} />
                                <div style={{ display: 'flex', gap: '6px' }}>
                                    <button onClick={() => setShowCreditForm(false)} className="btn-ghost" style={{ flex: 1, padding: '8px', fontSize: '12px' }}>Cancelar</button>
                                    <button onClick={handleCredit} className="btn-accent" disabled={!amount}
                                        style={{ flex: 2, padding: '8px', fontSize: '12px', opacity: !amount ? 0.4 : 1 }}>
                                        Confirmar
                                    </button>
                                </div>
                            </div>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--bg-surface)', flexShrink: 0 }}>
                {(['history', 'credit'] as const).map(t => (
                    <button key={t} onClick={() => onTabChange(t)} style={{
                        flex: 1, padding: '12px', fontSize: '13px', fontWeight: 500, cursor: 'pointer',
                ...
                {activeTab === 'history' ? (
                                ...
                                <div key={p.id} style={{
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'flex', gap: '10px' }}>
                                            ...
                                            <span style={{
                                                background: 'var(--bg-overlay)', padding: '1px 7px', borderRadius: '20px',
                                                fontSize: '10px', fontWeight: 500, color: 'var(--text-secondary)',
                                            }}>{p.payment}</span>
                                        </div>
                                    </div>
                                    <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '14px', color: 'var(--accent)' }}>
                                        {formatCRC(p.total)}
                                    </span>
                                </div>
                            ))
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {c.creditHistory.length === 0
                            ? <div style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)', fontSize: '13px' }}>Sin movimientos en cuenta fiado</div>
                            : [...c.creditHistory].reverse().map(tx => (
                                <div key={tx.id} style={{
                                    ...
                                    <div style={{
                                        ...
                                        background: tx.type === 'payment' ? 'var(--success-bg)' : 'var(--danger-bg)',
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                    }}>
                                        {tx.type === 'payment'
                                    ...
                                    <span style={{
                                        ...
                                        color: tx.type === 'payment' ? 'var(--success)' : 'var(--danger)',
                                    }}>
                                        {tx.type === 'payment' ? '−' : '+'}  {formatCRC(tx.amount)}
                                    </span>
                                </div>
                            ))
                        }
...
function CustomerModal({ customer, onSave, onClose }: {
    ...
    const set = (k: keyof Customer, v: string | number) =>
        setForm(f => ({ ...f, [k]: v }))
        ...
        <div style={{
            ...
            <div className="card animate-pop" style={{ width: 440, padding: '24px', maxHeight: '90vh', overflowY: 'auto' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <div>
                        ...
                        <input className="input-base" value={form.name} onChange={e => set('name', e.target.value)}
                            placeholder="María Rodríguez" style={{ width: '100%', padding: '9px 12px', fontSize: '13px' }} />
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                        <div>
                            ...
                            <input className="input-base" value={form.phone} onChange={e => set('phone', e.target.value)}
                                placeholder="8888-1234" style={{ width: '100%', padding: '9px 12px', fontSize: '13px' }} />
                        </div>
                        <div>
                            ...
                            <input className="input-base" type="email" value={form.email ?? ''} onChange={e => set('email', e.target.value)}
                                placeholder="correo@gmail.com" style={{ width: '100%', padding: '9px 12px', fontSize: '13px' }} />
                        </div>
                    </div>

                    {/* Estado y límite crédito */}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                        <div>
                        ...
                        <div>
                            ...
                            <input className="input-base" type="number" value={form.creditLimit} onChange={e => set('creditLimit', Number(e.target.value))}
                                placeholder="0 = sin crédito" style={{ width: '100%', padding: '9px 12px', fontSize: '13px' }} />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea className="input-base" value={form.notes ?? ''} onChange={e => set('notes', e.target.value)}
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '20px' }}>
                    ...
                    <button onClick={() => onSave(form)} className="btn-accent"
                        disabled={!form.name || !form.phone}
                        style={{ flex: 2, padding: '11px', opacity: (!form.name || !form.phone) ? 0.4 : 1 }}>
