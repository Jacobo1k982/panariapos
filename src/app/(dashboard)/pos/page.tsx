    ...
    const CatalogPanel = (
        <div style={{
            ...
            <div style={{
                ...
                {loadingCats ? (
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)', padding: '4px 0' }}>Cargando...</div>
                    ...
                    (products as any[]).map((product, i) => {
                        ...
                        return (
                            <div
                                ...
                                onClick={() => {
                                    addItem({ ...product, price: Number(product.price), taxRate: 0, stock: 99 })
                                    // En móvil dar feedback visual sin cambiar de tab
                                }}
                                style={{
                            ...
                            >
                                ...
                                <div style={{ fontSize: '13px', fontWeight: 500, lineHeight: '1.3', marginBottom: '6px' }}>
                                    {product.name}
                                </div>
                                <div style={{ fontSize: '10px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginBottom: '5px' }}>
                                    {product.sku}
                                </div>
        ...
        <div style={{
            ...
            <div style={{
                ...
                {cart.length === 0 ? (
                    <div style={{
                        ...
                        <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Tocá un producto para agregar</span>
                    </div>
                ) : (
                    cart.map(item => (
                        <CartItemRow key={item.product.id} item={item} onQty={updateQty} onRemove={removeItem} />
                    ))
                )}
            </div>
            ...
            <div style={{ borderTop: '1px solid var(--border)', padding: '14px 16px', flexShrink: 0 }}>
                ...
                <div style={{ marginBottom: '10px' }}>
                    {showDiscountInput ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <button onClick={() => setShowDiscountInput(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px' }}>
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                        ...
                        <span style={{
                            ...
                            letterSpacing: '-0.5px', color: total > 0 ? 'var(--accent)' : 'var(--text-muted)',
                        }}>
                            {formatCRC(total)}
                        </span>
                    </div>
                ...
                >
                    ...
                    Cobrar
                    {total > 0 && <span style={{ fontFamily: 'var(--font-mono)' }}>{formatCRC(total)}</span>}
        ...
        <>
            ...
            <div className="hide-desktop" style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
                ...
                <div style={{
                    ...
                    <button
                        ...
                        style={{
                            ...
                            color: mobileTab === 'catalog' ? 'var(--accent)' : 'var(--text-muted)',
                            borderBottom: mobileTab === 'catalog' ? '2px solid var(--accent)' : '2px solid transparent',
                    ...
                    <button
                        ...
                        style={{
                            ...
                            background: 'none', border: 'none', cursor: 'pointer',
                            fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-sans)',
                            color: mobileTab === 'cart' ? 'var(--accent)' : 'var(--text-muted)',
                            borderBottom: mobileTab === 'cart' ? '2px solid var(--accent)' : '2px solid transparent',
                            transition: 'all 0.15s',
                            position: 'relative',
                        }}
                    >
                ...
                <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                    {mobileTab === 'catalog' ? (
                        // En móvil el catálogo ocupa todo el ancho
                                    ...
                                    (products as any[]).map((product, i) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                onClick={() => {
                                                    addItem({ ...product, price: Number(product.price), taxRate: 0, stock: 99 })
                                                    // Cambiar al carrito automáticamente al agregar en móvil
                                                    setMobileTab('cart')
                                                }}
                                                style={{
                                                    padding: '13px 11px', cursor: 'pointer',
                        ...
                        // Carrito en móvil — ocupa todo el ancho
                            ...
                            <div style={{
                                ...
                                {cart.length === 0 ? (
                                    <div style={{
                                        flex: 1, display: 'flex', flexDirection: 'column',
                                        alignItems: 'center', justifyContent: 'center',
                                        gap: '10px', color: 'var(--text-muted)', paddingTop: '60px',
                                    }}>
                                        <span style={{ fontSize: '40px' }}>🛒</span>
                                        <span style={{ fontSize: '13px' }}>El carrito está vacío</span>
                                        <button
                                            onClick={() => setMobileTab('catalog')}
                                            className="btn-ghost"
                                            style={{ padding: '8px 20px', fontSize: '13px', marginTop: '8px' }}
                                        >
                                            ← Ver productos
                                        </button>
                                    </div>
                            ...
                            <div style={{ borderTop: '1px solid var(--border)', padding: '14px 16px', flexShrink: 0 }}>
                                {showDiscountInput ? (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                                        ...
                                        />
                                        <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>%</span>
                                        <button onClick={() => setShowDiscountInput(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                                            <X size={12} color="var(--text-muted)" />
                                        </button>
                                    </div>
                                ) : (
                                    ...
                                    >
                                        ...
                                        Agregar descuento global
                                        {globalDiscount > 0 && (
                                            <span style={{ color: 'var(--accent)', fontWeight: 600, marginLeft: 'auto' }}>{globalDiscount}%</span>
                                        )}
                                    </button>
                                )}
                                ...
                                >
                                    {createSale.isPending ? <Loader size={16} style={{ animation: 'spin 1s linear infinite' }} /> : null}
                                    Cobrar {total > 0 && <span style={{ fontFamily: 'var(--font-mono)' }}>{formatCRC(total)}</span>}
                                    <ChevronRight size={16} />
                                </button>
    ...
    return (
        <div className="animate-slide" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '8px' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: '14px', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: '14px', minWidth: '22px', textAlign: 'center' }}>
                        {item.quantity}
                    </span>
                    <button
                        onClick={() => onQty(item.product.id, item.quantity + 1)}
